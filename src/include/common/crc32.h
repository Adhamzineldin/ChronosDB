 ...
 * 
 * Algorithm: CRC-32/ISO-HDLC (same as zlib, gzip, PNG)
 */
class CRC32 {
public:
    /**
    ...
    /**
     * Compute CRC32 incrementally (for streaming data)
     * 
     * @param crc Previous CRC value (use 0xFFFFFFFF for initial)
     * @param data Pointer to data
     * @param length Length in bytes
    ...
    /**
     * Finalize incremental CRC computation
     */
    ...
    static bool Verify(const void* data, size_t length, uint32_t expected) {
        return Compute(data, length) == expected;
    }
