...
namespace francodb {
    ...
    std::unique_ptr<Statement> Parser::ParseQuery() {
        ...
        // 9. RECOVERY COMMANDS [NEW]
        else if (current_token_.type == TokenType::CHECKPOINT) {
            Advance(); // Eat CHECKPOINT
            if (!Match(TokenType::SEMICOLON))
        ...
        // 14. SHOW ...
            ...
            if (current_token_.type == TokenType::USER) {
                ...
                Advance();
                if (current_token_.type != TokenType::TABLE) throw Exception(ExceptionType::PARSER, "Expected TABLE");
                Advance();
                auto stmt = std::make_unique<ShowCreateTableStatement>();
                if (current_token_.type != TokenType::IDENTIFIER) throw Exception(
                    ExceptionType::PARSER, "Expected table name");
