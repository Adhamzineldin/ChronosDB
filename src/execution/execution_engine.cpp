...
namespace francodb
{
    ...
    ExecutionResult ExecutionEngine::Execute(Statement* stmt)
    {
        ...
        try
        {
            ...
            case StatementType::CREATE_INDEX:
                res = ExecuteCreateIndex(dynamic_cast<CreateIndexStatement*>(stmt));
                break;
            case StatementType::CREATE:
                res = ExecuteCreate(dynamic_cast<CreateStatement*>(stmt));
                break;
            case StatementType::INSERT:
                res = ExecuteInsert(dynamic_cast<InsertStatement*>(stmt));
                break;
            case StatementType::SELECT:
                res = ExecuteSelect(dynamic_cast<SelectStatement*>(stmt));
                break;
            ...
            case StatementType::SHOW_DATABASES:
                res = ExecuteShowDatabases(dynamic_cast<ShowDatabasesStatement*>(stmt));
                break;
            case StatementType::SHOW_TABLES:
                res = ExecuteShowTables(dynamic_cast<ShowTablesStatement*>(stmt));
                break;
            case StatementType::SHOW_STATUS:
                res = ExecuteShowStatus(dynamic_cast<ShowStatusStatement*>(stmt));
                break;
            case StatementType::WHOAMI:
                res = ExecuteWhoAmI(dynamic_cast<WhoAmIStatement*>(stmt));
                break;
    ...
    {
        if (!current_transaction_ || !in_explicit_transaction_) return ExecutionResult::Error(
    ...
    ExecutionResult ExecutionEngine::ExecuteShowUsers(ShowUsersStatement* stmt)
    {
        ...
        for (const auto& user : users)
        {
            ...
            for (const auto& pair : user.db_roles)
            {
                ...
                switch (pair.second)
                {
                case UserRole::SUPERADMIN: roles += "SUPERADMIN";
                    break;
                case UserRole::ADMIN: roles += "ADMIN";
                    break;
                case UserRole::USER: roles += "USER";
                    break;
                case UserRole::READONLY: roles += "READONLY";
                    break;
                case UserRole::DENIED: roles += "DENIED";
                    break;
                }
                roles += "@" + pair.first;
            }
