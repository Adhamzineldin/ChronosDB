# ChronosDB Installer Guide - FULLY REDESIGNED ✨

## Overview
The ChronosDB installer has been **completely redesigned** to provide a professional, production-ready installation experience with full configuration wizard, Windows Service integration, and proper system setup.

## 🎯 What's New

### ✅ Professional Installation Wizard
1. **Welcome Page** - Clear introduction with feature overview
2. **Port Configuration** - Interactive port selection with validation (1024-65535)
3. **Administrator Credentials** - Secure username/password setup with validation
4. **Encryption Options** - Three choices:
   - No encryption (fastest, less secure)
   - Auto-generated 256-bit key (recommended) - Generates and displays key
   - Custom encryption key (advanced) - User provides their own key
5. **Input Validation** - All fields validated before proceeding

### ✅ Windows Service (NEW!)
- **Replaced unreliable VBS/BAT scripts with proper Windows Service**
- Service name: `ChronosDBService`
- Auto-starts on Windows boot
- Automatic crash detection and restart
- Graceful shutdown handling (tries Ctrl+C, then terminate)
- Windows Event Log integration for monitoring
- Proper service lifecycle management

### ✅ System Integration
- ✅ Adds `bin\` to system PATH (run `chronosdb_shell` from anywhere)
- ✅ Creates Start Menu shortcuts (Shell, Config Editor, Uninstaller)
- ✅ Auto-creates data directory
- ✅ Generates configuration file from user input
- ✅ Modern installer UI with icon support

### ❌ Removed
- `start_server.vbs` (unreliable)
- `stop_server.bat` (unreliable)  
- `refresh_env.bat` (not needed)

## 🔧 Building the Installer

### Prerequisites
- **CMake** 3.10+
- **C++ Compiler** (MinGW-GCC or MSVC)
- **Inno Setup** from https://jrsoftware.org/isdl.php

### Step 1: Build Executables
```bash
# Configure for Release
cmake -B cmake-build-release -DCMAKE_BUILD_TYPE=Release

# Build all targets
cmake --build cmake-build-release --config Release -j 4
```

**Output:**
- `chronosdb_server.exe` - Database server
- `chronosdb_shell.exe` - Interactive CLI
- `chronosdb_service.exe` - **NEW** Windows service wrapper
- `chronosdb_setup.exe` - Standalone setup tool (optional)

### Step 2: Compile Installer
1. Open `installer.iss` in Inno Setup Compiler
2. Click **Build** → **Compile**
3. Output: `Output/ChronosDB_Setup.exe`

## 📦 Installation Process

### User Experience
1. **Run** `ChronosDB_Setup.exe` (requires Administrator)
2. **Welcome** - See what will be installed
3. **Select Directory** - Default: `C:\Program Files\ChronosDB`
4. **Configure Port** - Default 2501, validates availability
5. **Set Credentials** - Username (min 3 chars), Password (min 4 chars)
6. **Choose Encryption:**
   - None
   - Auto-generate key → **Shows popup with key to save!**
   - Custom key (64 hex chars)
7. **Install** - Copies files, creates config, registers service
8. **Start Service** - Automatically starts ChronosDB

### What Gets Installed
```
C:\Program Files\ChronosDB\
├── bin\
│   ├── chronosdb_server.exe    (Database engine)
│   ├── chronosdb_shell.exe     (Client shell)
│   ├── chronosdb_service.exe   (Service wrapper)
│   └── chronosdb.conf          (Generated config)
└── data\                      (Database files)
```

### Configuration File Generated
```ini
# ChronosDB Configuration File
# Generated by ChronosDB Installer
# 2026-01-14 22:30:00

port = 2501
root_username = "admin"
root_password = "your_secure_password"
data_directory = "../data"
encryption_enabled = true
encryption_key = "auto_generated_64_char_hex_key_here_if_enabled"
autosave_interval = 30
```

## 🚀 Using ChronosDB

### Connect to Database
```bash
# From anywhere (added to PATH):
chronosdb_shell

# Connect with credentials
chronosdb_shell -h localhost -p 2501 -u admin -P your_password
```

### Service Management
```cmd
# Start service
sc start ChronosDBService

# Stop service
sc stop ChronosDBService

# Check status
sc query ChronosDBService

# Restart service
sc stop ChronosDBService && sc start ChronosDBService
```

### View Logs
1. Open **Event Viewer** (eventvwr.msc)
2. Navigate to **Windows Logs** → **Application**
3. Filter by Source: **ChronosDBService**

## 🔒 Security Notes

### Encryption Key Management
- **Auto-generated keys are shown ONCE during installation**
- Save the key immediately in a secure location
- Without the key, encrypted data cannot be recovered
- Key is stored in `chronosdb.conf` (secure this file!)

### Credentials
- Root password is stored in config file (hashed in future version)
- Use strong passwords (current minimum: 4 chars)
- Secure the configuration file with NTFS permissions

### Service Security
- Service runs as LOCAL SYSTEM (required for Windows Service)
- Data directory should have restricted permissions
- Consider firewall rules for port access

## 🐛 Troubleshooting

### Service Won't Start
1. Check Event Viewer for errors
2. Verify config file exists and is valid
3. Check port availability: `netstat -ano | findstr :2501`
4. Ensure data directory is writable

### Port Already in Use
```bash
# Find process using port
netstat -ano | findstr :2501

# Change port in config
notepad "C:\Program Files\ChronosDB\bin\chronosdb.conf"

# Restart service
sc stop ChronosDBService && sc start ChronosDBService
```

### Can't Connect
- Verify service is running: `sc query ChronosDBService`
- Check firewall settings
- Verify credentials in `chronosdb.conf`
- Try connecting locally first: `chronosdb_shell -h localhost`

## 🏗️ Technical Details

### Service Architecture
```
Windows Service Controller
    ↓ manages
ChronosDBService (chronosdb_service.exe)
    ↓ spawns & monitors
chronosdb_server.exe
    ↓ reads
chronosdb.conf
    ↓ stores data in
data/chronosdb.db
```

### Service Wrapper Features
- Process monitoring with 1-second check interval
- Automatic restart on crash
- Graceful shutdown (10-second timeout)
- Event logging for all operations
- Handles Ctrl+C, SIGTERM, console close events

### Files Modified
- ✅ `installer.iss` - Complete rewrite with 5 wizard pages
- ✅ `src/cmd/service/service_wrapper.cpp` - NEW Windows service
- ✅ `CMakeLists.txt` - Added service build target
- ✅ `INSTALLER_README.md` - Complete documentation

## 📝 Uninstallation
1. Use **Add or Remove Programs** in Windows Settings
2. Or run uninstaller from Start Menu
3. Service is automatically stopped and removed
4. Files and data directory are deleted
5. PATH is cleaned up

## 🚀 Next Steps
After installation:
1. ✅ Service is running
2. ✅ Open `chronosdb_shell` from any terminal
3. ✅ Connect with your credentials
4. ✅ Start creating databases!

## 📚 Further Reading
- See `docs/` for database usage guide
- Check `examples/` for SDK usage (C++, Python, Node.js)
- Visit project wiki for advanced topics

## 🙌 Credits
ChronosDB - High-performance relational database management system
