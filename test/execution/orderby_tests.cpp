...
void RunOrderByTests(TestRunner& runner) {
    ...
    runner.RunTest("OrderBy", "Large_Dataset", []() {
        ...
        for (int i = 1000; i > 0; i--) large_data.push_back(i);
        std::sort(large_data.begin(), large_data.end());
        ASSERT_EQ(large_data.front(), 1, "First element should be 1");
        ASSERT_EQ(large_data.back(), 1000, "Last element should be 1000");
    });
